...
const funcModule = {
    ...
    addPhotoPost(photoPost) {
        if (this.validatePhotoPost(photoPost) && photoPosts
            .findIndex(elem => elem.id === photoPost.id) === -1) {
            photoPosts = JSON.parse(fs.readFileSync(postsPath), (key, value) => {
                if (key === 'createdAt') return new Date(value);
                return value;
            });
            photoPosts.push(photoPost);
            photoPosts.sort((elem1, elem2) => new Date(elem2.createdAt)
                - new Date(elem1.createdAt));
            fs.writeFileSync(postsPath, JSON.stringify(photoPosts));
            fs.writeFileSync('./server/data/timePageID.json', `{"id":"${(parseInt(photoPost.id, 10) + 1)}"}`);
            return true;
        }
